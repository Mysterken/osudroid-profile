...
export async function fetchUserFromApi(uid: string) {
	const key = `osudroid:user:uid:${uid}`;
...
export async function fetchLeaderboardFromApi(
	...
	const key = `osudroid:leaderboard:${type}:${region}:p${page}:l${limit}`;
	const ttl = 3 * 60 * 1000; // 3 minutes

	return wrapCache(key, ttl, async () => {
		try {
			logger.debug(
				`Fetching leaderboard from osu!droid API: ${type}, ${region}, ${page}, ${limit}`
			);
			...
			logger.error({ error }, `❌ Unexpected error fetching leaderboard ${type} ${region}`);
			throw new ApiError(`Unexpected error fetching leaderboard ${type} ${region}`);
		}
	});
}
...
export async function fetchBeatmapScoresFromApi(
	...
	const key = `osudroid:beatmap:scores:${hash}:order=${order}:page=${page}`;
